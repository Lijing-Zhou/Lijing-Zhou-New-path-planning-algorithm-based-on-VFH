...
t=receive(camera);
% the robot will rotate to find the ball
velmsg.Linear.X=0;
...
% RGP idnetification of the picture to find the ball with color and shape
m=0;
R=t.Data(3:3:end);
R=reshape(R,[960,540]);
R=R';
...
%red find the red aera
area=(H>0.96 & H<1)&(S>0.4 &S<1) ;%red HSV valeve
H(area)=1;
H(~area)=0;
% H(1:340,:)=1;
b=imfill(H,'holes');
...
% Find the cornor( the destination of the ball) result: positio
while 1
    disp("Find the cornor")
   camera = rossubscriber('/kinect2/qhd/image_color/');
lasermsg=receive(laser);
...
% the cornor have a different color and it will help to find the closest
% position
R=t.Data(3:3:end);
R=reshape(R,[960,540]);
R=R';
...
t=receive(camera);
if d==1
    disp("zuo daoqiang no obstacle");
    velmsg.Linear.X=0;
velmsg.Angular.Z = -0.2;
% If there is an obstacle
elseif d==2
    disp("you daoqiang If there is an obstacle");
    velmsg.Linear.X=0;
velmsg.Angular.Z = 0.2;
...
% Move to the corner when there are obstacles         (zhangaiwu daoqiang)
while d
...
% the later code is to repeat the same work
for n=1:3
%zhao qiu
while 1
%     disp("zhao qiu")
